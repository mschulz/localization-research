...
week-covered: 2026-08-01 to 2026-08-06
new-files-in-raw: 0
...
# Weekly Intelligence Summary — 2026-08-06

## Summary

**No new files were added to `raw/` this week.** The window since the last run (2026-08-01 to 2026-08-06) was checked three ways — by modification time (`find -newermt 2026-07-30`), by extending the window back to 2026-07-13, and via git history (`git log --diff-filter=A`). None show any new source activity in `raw/`. The most recent source in the vault remains dated 2026-06-15 (the mid-June import). No compilation was triggered; the knowledge base is unchanged since the last run.
...
All 22 articles in `knowledge/` were scanned for JSON-style frontmatter that would need conversion to YAML. **None found — every article uses valid YAML frontmatter** (`tags`, `date-compiled`, `source-files`, `status`). This summary is likewise written with YAML frontmatter. No conversion was necessary this cycle.
...
- 79 content-bearing PDFs in `raw/` (plus the 0-byte Chen 2006 file, excluded)
- Last health check: 2026-08-03 — see `outputs/health-check-2026-08-03.md`

## Report Retention (observational — no deletions made)

The weekly-file-check task file does not instruct deletion of old reports, so none were removed. For visibility: the two 2026-07-04 reports (`weekly-intelligence-summary-2026-07-04.md` and `weekly-summary-2026-07-04.md`) are now more than one month old. If a retention policy is desired, these are the candidates to archive; flagging only.
